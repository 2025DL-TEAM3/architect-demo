...
# input paths
base_model = 'Qwen/Qwen3-4B'  # auto-downloaded from huggingface.co
arc_data_path = os.path.join('input', 'my-arc')  # as on kaggle arc prize 2024
...
# output paths
save_model_path = os.path.join('pretrained_models', "Qwen3-4B-Finetuned")
...
for action in ['train', 'merge']:
    ...
    if action == 'train':
        # load training data
        arc_eval_set = ArcDataset.load_from_json(os.path.join(arc_data_path, 'arc-agi_my_evaluation_challenges.json'))
        arc_eval_set = arc_eval_set.load_solutions(os.path.join(arc_data_path, 'arc-agi_my_evaluation_solutions.json'))
        ...
        # }
        train_dataset = ArcDataset.load_from_json(
            os.path.join(arc_data_path, 'arc-agi_my_training_challenges.json')
        ).load_solutions(
            os.path.join(arc_data_path, 'arc-agi_my_training_solutions.json')
        )
